...
import {parseRoute,courseRoute,choose,finalIncident} from './interaction.mjs';
...
const routeState=ref(parseRoute(location.hash)||{view:'map'}),selected=ref(-1),result=ref(null);
...
function sync(){routeState.value=parseRoute(location.hash)||{view:'map'};selected.value=-1;result.value=null;window.scrollTo({top:0,behavior:'instant'});nextTick(()=>pageHeading.value?.focus())}
...
<template><div class="v2-shell"><header class="v2-header"><a :href="courseRoute()" class="v2-brand"><b>System Design</b><span>互動式學習地圖</span></a><a :href="courseRoute()" class="v2-map-link">八個問題</a></header><main>
<section v-if="routeState.view==='map'" class="v2-home"><p class="v2-kicker">SYSTEM DESIGN · LEARNING MAP</p><h1 ref="pageHeading" tabindex="-1">{{course.subtitle}}</h1><p class="v2-lead">{{course.intro}}</p><a class="v2-primary" :href="courseRoute(units[0].id,units[0].stages[0].id)">開始第一章 →</a><div class="v2-unit-grid"><a v-for="u in units" :key="u.id" :href="u.stages.length?courseRoute(u.id,u.stages[0].id):courseRoute(u.id)" :class="['v2-unit-card',{'is-pending':!u.stages.length}]"><span>{{String(u.number).padStart(2,'0')}}</span><h2>{{u.title}}</h2><p>{{u.summary}}</p><small>{{u.stages.length?'可開始學習':'製作中'}}</small></a><a class="v2-unit-card v2-final-card" href="#/final"><span>FINAL</span><h2>沒有章節提示</h2><p>把八章的推理帶進案件追蹤＋批次文件處理。</p><small>整合 Transfer</small></a></div></section>
<section v-else-if="routeState.view==='final'&&incident" class="v2-lesson"><nav class="v2-breadcrumb"><a :href="courseRoute()">八個問題</a><span>／</span><span>Final Transfer</span><span>／</span><span>{{incidentIndex+1}} / {{finalTransfer.incidents.length}}</span></nav><p class="v2-kicker">沒有章節提示</p><h1 ref="pageHeading" tabindex="-1">{{incident.title}}</h1><p class="v2-lead">{{finalTransfer.intro}}</p><FinalIntegratedSimulator :incident-id="incident.id" /><div class="v2-actions"><a v-if="nextIncident" class="v2-primary" :href="courseRoute('final',nextIncident.id)">注入下一個事故 →</a><a v-else class="v2-primary" :href="courseRoute()">完成第一輪 →</a></div></section>
<section v-else-if="routeState.view==='notfound'" class="v2-empty"><h1 ref="pageHeading" tabindex="-1">找不到這一頁</h1><p class="v2-lead">請從八個問題重新選擇。</p><a class="v2-primary" :href="courseRoute()">回到八個問題 →</a></section>
<section v-else-if="unit&&!stage" class="v2-empty"><p class="v2-kicker">UNIT {{unit.number}}</p><h1>{{unit.title}}</h1><p class="v2-lead">{{unit.summary}}</p><p class="v2-note">這個單元正在製作中。你可以先從前兩章開始。</p><a class="v2-secondary" :href="courseRoute()">← 回八個問題</a></section>
<article v-else-if="stage" class="v2-lesson"><nav class="v2-breadcrumb"><a :href="courseRoute()">八個問題</a><span>／</span><span>Unit {{unit.number}}</span><span>／</span><span>{{stageIndex+1}} / {{unit.stages.length}}</span></nav><p class="v2-kicker">{{stage.eyebrow}}</p><h1 ref="pageHeading" tabindex="-1">{{stage.title}}</h1><p class="v2-lead">{{stage.intro}}</p><LessonLab :unit-id="unit.id" :stage-id="stage.id" />
<section class="v2-board v2-reflection"><div class="v2-question"><span>現在先想</span><h2>{{stage.prompt}}</h2></div><div v-if="stage.id!=='transfer'" class="v2-options"><button v-for="(option,i) in stage.options" :key="option.label" :aria-pressed="selected===i" @click="select(i)"><span>{{option.label}}</span><b>→</b></button></div><div v-if="result" class="v2-result" role="status"><p class="v2-result-label">看看這個選擇帶來什麼</p><p>{{result.feedback}}</p><div v-if="result.term" class="v2-term"><strong>{{result.term.name}}</strong><span>{{result.term.plain}}</span></div></div><p v-else-if="stage.id==='transfer'" class="v2-note">改動上方情境的設定，觀察系統狀態與結果，再用自己的話回答這個問題。</p><p v-else class="v2-note">沒有計分。先做一個判斷，再看它會把設計帶到哪裡。</p></section><div class="v2-actions"><button class="v2-secondary" @click="resetExperiment">重設本章實驗</button><button v-if="result" class="v2-secondary" @click="restart">清除反思選擇</button><a v-if="nextStage" class="v2-primary" :href="courseRoute(unit.id,nextStage.id)">繼續：{{nextStage.title}} →</a><a v-else class="v2-primary" :href="courseRoute()">完成本章 →</a></div></article>
