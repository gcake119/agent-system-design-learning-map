...
import RequirementSimulator from './components/RequirementSimulator.vue';
import BoundarySimulator from './components/BoundarySimulator.vue';
...
<article v-else-if="stage" class="v2-lesson"><nav class="v2-breadcrumb"><a :href="v2Route()">八個問題</a><span>／</span><span>Unit {{unit.number}}</span><span>／</span><span>{{stageIndex+1}} / {{unit.stages.length}}</span></nav><p class="v2-kicker">{{stage.eyebrow}}</p><h1>{{stage.title}}</h1><p class="v2-lead">{{stage.intro}}</p><RequirementSimulator v-if="unit.id==='requirements' && stage.id==='enough'" />
<BoundarySimulator v-else-if="unit.id==='boundaries' && stage.id==='responsibility'" />
<ConcurrencySimulator v-else-if="unit.id==='concurrency' && stage.id==='race'" />
